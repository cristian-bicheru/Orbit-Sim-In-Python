    ...
    def update(self):
        if self.dT == 0:
            ...
            if centerY - HEIGHT/2 > 0:
                gravMagnitude = decimal.Decimal(gravConstant*blackHoleMass/((centerX-WIDTH/2)**2+(centerY-HEIGHT/2)**2))
                aByc = decimal.Decimal((centerX-WIDTH/2) / math.sqrt((centerX-WIDTH/2)**2+(centerY-HEIGHT/2)**2))
                gravXacceleration = -(gravMagnitude*(aByc))
                gravYacceleration = -(gravMagnitude*decimal.Decimal(math.cos(math.asin(aByc))))
            ...
            dy = (decimal.Decimal(self.speedy)*decimal.Decimal(self.dT))+decimal.Decimal(0.5)*gravYacceleration*decimal.Decimal(self.dT)**2
            dx = (decimal.Decimal(self.speedx)*decimal.Decimal(self.dT))+decimal.Decimal(0.5)*gravXacceleration*decimal.Decimal(self.dT)**2
            self.speedy = decimal.Decimal(self.speedy)+(gravYacceleration*decimal.Decimal(self.dT))
            self.speedx = decimal.Decimal(self.speedx)+gravXacceleration*decimal.Decimal(self.dT)
